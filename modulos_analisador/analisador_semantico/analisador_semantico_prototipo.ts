...
/**
 * Interface para um nó da árvore sintática
 */
interface TreeNode {
    nome: string;
    ordem: number;
 ...
 */
function adicionarErro(estado: EstadoSemantico, mensagem: string): void {
...
function analisarExpressao(
    ...
    for (const filho of no.filhos) {
        if (filho.nome === "termo") {
            ...
            operadores.push(filho.filhos[0].nome);
        }
    }
...
function analisarChamadaFuncao(
    ...
    if (nomeFuncao) {
        ...
        if (!simbolo) {
            adicionarErro(estado, `Função '${nomeFuncao}' não declarada`);
            return { tipo: "void" }; // Retorno padrão para evitar erros em cascata
        } else if (!simbolo.isFunction) {
            adicionarErro(estado, `'${nomeFuncao}' não é uma função`);
            return { tipo: "void" };
        } else {
...
function analisarAtribuicao(estado: EstadoSemantico, no: TreeNode): void {
    ...
    if (identificador) {
        const simbolo = buscarSimbolo(estado, identificador);
        if (!simbolo) {
            adicionarErro(estado, `Variável '${identificador}' não declarada`);
        } else if (simbolo.isFunction) {
            adicionarErro(
                estado,
...
function analisarDeclaracaoReturn(estado: EstadoSemantico, no: TreeNode): void {
    ...
    if (!estado.escopoFuncaoAtual) {
        adicionarErro(estado, "Instrução 'return' fora de uma função");
        return;
    }
    ...
    for (const filho of no.filhos) {
        if (filho.nome === "TYPE" && filho.filhos.length > 0) {
            ...
            if (isTipoValido(tipoValor)) {
                ...
                adicionarErro(estado, `Tipo de retorno inválido: ${tipoValor}`);
            }
        } else if (filho.nome === "IDENTIFIER" && filho.filhos.length > 0) {
            nome = filho.filhos[0].nome;
    ...
    for (const filho of no.filhos) {
        if (filho.nome === "TYPE" && filho.filhos.length > 0) {
            ...
            if (isTipoValido(tipoValor)) {
                ...
                adicionarErro(estado, `Tipo inválido: ${tipoValor}`);
            }
        } else if (filho.nome === "IDENTIFIER" && filho.filhos.length > 0) {
            nome = filho.filhos[0].nome;
        } else if (
